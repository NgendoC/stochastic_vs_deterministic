...
\begin{align*}
min(1, \frac{P(x \mid \theta')}{P(x \mid \theta)} \frac{q(\theta \mid \theta')}{q(\theta' \mid \theta)})
\end{align*}
where q refers to the proposal distribution. If the ratio is more than 1, the proposed change is accepted while if the ratio is below 1, the proposed change is rejected.
...
\begin{figure}[!h]
\begin{center}
\includegraphics{heatmap_writeup}

...
I have done all the programming myself, except in situations where stated that I have used a package, for example deSolve for solving differential equations for deterministic models.
...
For population size 50, the 95\% interval overestimates the true $\beta$ value for all five simulations and one out of five simulations when $R_{0}$ is 1.5 and 6 respectively (Table 1). The remaining four simulations for the higher $R_{0}$, on the other hand, overestimate $\beta$. The simulations with an $R_{0}$ of 1.5 overestimate $\gamma$ for simulations 1 and 4, with the remaining three simulations underestimating $\gamma$. $\gamma$ is included in the 95\% intervals for simulations 1 and 4 for the outbreaks with a $R_{0}$ of 6, while simulations 2 and 5 overestimate $\gamma$ and simulation 3 underestimates it.

...
Personalised paragraph: I have done all the programming myself, except in situations where stated that I have used a package, for example deSolve for solving differential equations for deterministic models.
...
The sum of squared differences between the model and the true recovery curve increases with population size (Table 4). Additionally, the sum of squared differences for the point estimates tend to be lower for the higher $R_{0}$ than the lower $R_{0}$ for the populations (Table 4). 
...
The 95\% credible intervals for $\beta$ and $\gamma$ for outbreaks with a population size of 200 and 3.5 million MCMC iterations all include the true values, except for the interval for $\beta$ in simulation 4, for the outbreakswhere $R_{0}$ is 1.5 (Table 6).

...
Tables 1, 2, and 3 show that when comparing the output of the residual error method to the parameter values used to generate the simulations that the RE method was being fit to, it can be seen that the residual error method consistently overestimates the values that $\beta$ might take for all population sizes when $R_{0}$ is 1.5 and consistently underestimates $\beta$ values when $R_{0}$ is 6. The inferred values of $\gamma$, on the other hand, include the true value used for the generating the simulations more often than with $\beta$, though levels of success vary with $R_{0}$ and population size (Table 1, 2, 3).
...
For population size 50, the 95\% interval overestimates the true $\beta$ value for all five simulations and one out of five simulations when $R_{0}$ is 1.5 and 6 respectively (Table 1). The remaining four simulations for the higher $R_{0}$, on the other hand, underestimate $\beta$. The simulations with an $R_{0}$ of 1.5 overestimate $\gamma$ for simulations 1 and 4, with the remaining three simulations underestimating $\gamma$. $\gamma$ is included in the 95\% intervals for simulations 1 and 4 for the outbreaks with a $R_{0}$ of 6, while simulations 2 and 5 overestimate $\gamma$ and simulation 3 underestimates it.

For outbreaks with a population size of 200, the 95\% interval again overerestimates $\beta$ for four out of five outbreaks where $R_{0}$ is 1.5, though simulation 1 includes the value in its interval (Table 2). For outbreaks with an $R_{0}$ of 6, $\beta$ is underestimated for all five simulations (Table 2). $\gamma$ is underestimated for two simulations, overestimated for one simulation, and is included in the 95\% interval for two simulations for the outbreaks where $R_{0}$ is 1.5. For outbreaks where $R_{0}$ is 6, $\gamma$ is included in the 95\% interval for one simulation and the remaining four simulations' intervals overestimate $\gamma$. 
...
A MCMC approach was also taken to infer the values that $\beta$ and $\gamma$ might take when given the recovery curve from the simulated outbreak. The MCMC with 3.5 million iterations took between 20 and 25 hours to run to completion while the MCMC with 17.5 iterations took between 120 hours and (5 to 7 days) to run to completion, with outbreaks with an $R_{0}$ of 1.5 taking longer to finish than outbreaks with an $R_{0}$ of 6.

For the outbreaks where population size was 50 and the MCMC was run for 3.5 million iterations, though the post-burn-in $\beta$ 95\% credible intervals include the true value for all outbreaks, simulation 5 from the outbreaks with an $R_{0}$ of 1.5 remains below the true $\gamma$ value (Table 5). The 95\% credible intervals for $\beta$ and $\gamma$ for outbreaks where $R_{0}$ is 6 all include the true values. 
...
The 95\% credible intervals for $\beta$ and $\gamma$ for outbreaks with a population size of 200 and 3.5 million MCMC iterations all include the true values, except for the interval for $\beta$ in simulation 4, where $R_{0}$ is 1.5 (Table 6).
...
For the 17.5 million iteration run for the largest population size, the MCMC did converge for the outbreak with an $R_{0}$ of 6 and population size of 1000 (Fig. X).

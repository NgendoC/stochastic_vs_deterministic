...
For the 17.5 million iteration MCMCs conducted for the outbreaks with a population size 1000 and an $R_{0}$ of 1.5, only simulations 1 and 5 included the true $\beta$, while the other three simulations overestimated the value. For $\gamma$, simulations 1, 4, and 5 include the true $\gamma$ while the other two simulations overestimate it. For outbreaks with and $R_{0}$ of 6, the true $\beta$ is included in the 95\% credible interval for all simulations except simulation 3. The true $\gamma$ is not included in the credible intervals of simulations 3 or 4, but is included in the intervals of the other three.

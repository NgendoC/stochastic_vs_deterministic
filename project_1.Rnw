...
\setlength{\parindent}{1em} % 5 space indent
...
For an $R_{0}$ of 1.5, the difference between the lower and upper bounds of the 95\% interval observed for $\beta$ and $\gamma$ were often wider than the differences for outbreaks with an $R_{0}$ of 6 (Table 1, Table 2, Table 3). This can also be seen in the exemplifying graphs (Fig. 5), where the infectious and recovery curves for the point estimate and bootstrapped parameter values for outbreaks with the larger $R_{0}$ are more consistent with each other with less variation in the curves amongst $\beta$ and $\gamma$ values obtained through bootstrapping. It also seems that the deterministic infectious curves resulting from the point estimate values of $\beta$ and $\gamma$ consistently result in an underestimation of the peak of the outbreak for the scenarios where $R_{0}$ is 6 (Fig. 5D-F). The $\beta$ estimates lower with increasing population size due to $\beta$ referring to the individual-level rate of infection (Table 1, Table 2, Table 3). 
...
Tables 1, 2, and 3 show that when comparing the output of the residual error method to the parameter values used to generate the simulations that the RE method was being fit to, the RE inference method consistently overestimates the values that $\beta$ might take for all population sizes when $R_{0}$ is 1.5 and consistently underestimates $\beta$ values when $R_{0}$ is 6. The inferred values of $\gamma$, on the other hand, include the true value used for the generating the simulations more often than for $\beta$, though levels of success vary with $R_{0}$ and population size (Table 1, 2, 3).
...
For population size 50, the 95\% interval overestimates the true $\beta$ value for all five simulations and one out of five simulations when $R_{0}$ is 1.5 and 6 respectively (Table 1). The remaining four simulations for the higher $R_{0}$, on the other hand, underestimate $\beta$. Simulations 1 and 4 for the outbreaks where $R_{0}$ of 1.5 overestimate $\gamma$, with the remaining three simulations underestimating $\gamma$. $\gamma$ is included in the 95\% intervals for simulations 1 and 4 for the outbreaks with a $R_{0}$ of 6, while simulations 2 and 5 overestimate $\gamma$ and simulation 3 underestimates it.

For outbreaks with a population size of 200, the 95\% interval again overerestimates $\beta$ for four out of five outbreaks where $R_{0}$ is 1.5, though simulation 1 includes the true value in its interval (Table 2). For outbreaks with an $R_{0}$ of 6, $\beta$ is underestimated for all five simulations (Table 2). $\gamma$ is underestimated for two simulations, overestimated for one simulation, and is included in the 95\% interval for two simulations for the outbreaks where $R_{0}$ is 1.5. For outbreaks where $R_{0}$ is 6, $\gamma$ is included in the 95\% interval for one simulation while the remaining four simulations' intervals overestimate $\gamma$. 
...
$\beta$ is also overestimated for most of the outbreaks where $R_{0}$ is 1.5 and the population size is 1000, only simulation 1's 95\% interval includes the true $\beta$ value (Table 3). Three out of the five simulations include the true $\gamma$ value in their 95\% intervals. When $R_{0}$ is 6, $\beta$ is underestimated for all five simulations, though all of them do include the true $\gamma$ value in their 95\% intervals (Table 3).  

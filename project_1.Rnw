...
\begin{titlepage}
    \begin{center}
        ...
        \textbf{Chance would be a fine thing: Comparing stochastic and deterministic SIR models for parameterisation and data augmentation}
...
The data available for model parameterisation should also be considered, though it will not be explored in this project. If many factors are unknown, a complex stochastic model making a multitude of assumptions might perform worse than a simpler deterministic model (mention HIV modelling example?). 
